import { app, BrowserWindow, ipcMain, shell, session, globalShortcut, dialog, Tray, Menu, nativeImage } from 'electron'
...
async function openNeteaseLoginWindow(owner: BrowserWindow | null): Promise<{ ok: boolean; cookie?: string; reused?: boolean }> {
  ...
  return new Promise((resolve) => {
    ...
    // v3.8.6 修复：监听页面导航（扫码成功后会跳转），立即检测 cookie，避免 1.2s 轮询延迟
    ...
    pollTimer = setInterval(checkCookies, 600)
...
// ========== 酷狗登录（v3.8.6 新增，与网易云完全隔离）==========
// 关键设计：用户 SVIP 的 vip_token/vip_type/dfid/KUGOU_API_MID 等字段
// 只能通过网页登录拿到（扫码接口只返回 token+userid，不够 SVIP 鉴权）
// 所以走和网易云一样的网页登录窗口方案，抓 .kugou.com 全域 cookie
const KUGOU_LOGIN_PARTITION = 'persist:aurorabeat-kugou'
const KUGOU_LOGIN_URL = 'https://www.kugou.com/'

function isKugouCookieDomain(domain: string): boolean {
  const d = (domain || '').replace(/^\./, '').toLowerCase()
  return d === 'kugou.com' || d.endsWith('.kugou.com')
}

function buildKugouCookieHeader(cookies: any[]): string {
  // 抓取 .kugou.com 域下所有 cookie（token/userid/vip_token/vip_type/dfid/kg_mid 等）
  // 这里不做优先级排序，全部带上，因为 song_url 需要的字段较多
  const picked = new Map<string, string>()
  ;(cookies || []).forEach((cookie) => {
    if (!cookie?.name || !isKugouCookieDomain(cookie.domain)) return
    picked.set(cookie.name, cookie.value || '')
  })
  return Array.from(picked.entries()).filter(([k, v]) => k && v).map(([k, v]) => `${k}=${v}`).join('; ')
}

async function readKugouCookie(cookieSession: Electron.Session): Promise<string> {
  const cookies = await cookieSession.cookies.get({})
  return buildKugouCookieHeader(cookies)
}

function kugouCookieHasLogin(cookieText: string): boolean {
  // 酷狗登录成功的标志：cookie 里同时有 token 和 userid
  return /(^|;)\s*token=/.test(cookieText) && /(^|;)\s*userid=/.test(cookieText)
}

async function openKugouLoginWindow(owner: BrowserWindow | null): Promise<{ ok: boolean; cookie?: string; reused?: boolean }> {
  const cookieSession = session.fromPartition(KUGOU_LOGIN_PARTITION)
  const initialCookie = await readKugouCookie(cookieSession)
  if (kugouCookieHasLogin(initialCookie)) {
    return { ok: true, cookie: initialCookie, reused: true }
  }
  ...
  return new Promise((resolve) => {
    ...
    const loginWindow = new BrowserWindow({
      width: 1000, height: 760, minWidth: 820, minHeight: 580,
      parent: owner && !owner.isDestroyed() ? owner : undefined,
      modal: false, show: false, autoHideMenuBar: true,
    ...
    const checkCookies = async () => {
      try {
        if (loginWindow.isDestroyed()) return
        const cookie = await readKugouCookie(cookieSession)
        if (kugouCookieHasLogin(cookie)) {
          finish({ ok: true, cookie })
        }
      } catch (e) { console.warn('Kugou cookie check failed:', e) }
    }

    // 酷狗网页登录后会跳转或刷新页面，监听导航事件立即检测
    loginWindow.webContents.on('did-navigate', () => checkCookies())
    loginWindow.webContents.on('did-navigate-in-page', () => checkCookies())
    loginWindow.webContents.on('did-frame-finish-load', () => checkCookies())

    // 点击外部链接（如注册、帮助）在系统浏览器打开，不在登录窗口跳转
    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
    ...
    loginWindow.on('closed', async () => {
      ...
      try {
        const cookie = await readKugouCookie(cookieSession)
        resolve(kugouCookieHasLogin(cookie) ? { ok: true, cookie } : { ok: false })
      } catch { resolve({ ok: false }) }
    })

    // 轮询检测 cookie（600ms，与网易云登录窗口一致）
    pollTimer = setInterval(checkCookies, 600)
    loginWindow.loadURL(KUGOU_LOGIN_URL).catch(() => finish({ ok: false }))
  })
}
...
app.whenReady().then(async () => {
  ...
  // v3.8.6 酷狗登录（独立 IPC，与网易云隔离）
  ipcMain.handle('kugou:openLogin', async () => {
    const result = await openKugouLoginWindow(mainWindow)
    if (result.ok && result.cookie && serverPort) {
      try {
        await fetch(`http://127.0.0.1:${serverPort}/api/kugou/login/cookie`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ cookie: result.cookie }),
        })
      } catch (e) { console.warn('Failed to send kugou cookie to server:', e) }
    }
    return result
  })

  ipcMain.handle('kugou:clearLogin', async () => {
    await clearKugouLoginSession()
    if (serverPort) {
      try { await fetch(`http://127.0.0.1:${serverPort}/api/kugou/logout`) } catch {}
    }
    return { ok: true }
  })
...
// v3.8.6: 标记是否真正退出（托盘"退出"或 Cmd+Q），区分于"关闭按钮最小化到托盘"
;(app as any).isQuitting = false

app.on('window-all-closed', () => {
  // v3.8.6: 不在 window-all-closed 时退出应用，让托盘保持运行
  // 主窗口隐藏后不算"关闭"，只有托盘"退出"才真正退出
